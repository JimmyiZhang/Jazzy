...
namespace Jazzy.Library
{
    ...
    public static class TypeExtension
    {
        ...
        {
            return member.GetCustomAttributes(typeof(T), inherit).Any(m => (m as T) != null);
        }
        ...
        /// <param name="member">类型类型成员</param>
        /// <param name="inherit">是否从继承中查找</param>
        /// <returns>存在返回第一个，不存在返回null</returns>
        public static T GetAttribute<T>(this MemberInfo member, bool inherit) where T : Attribute
        {
            return member.GetCustomAttributes(typeof(T), inherit).SingleOrDefault() as T;
        }
        ...
        /// <returns></returns>
        public static bool AsGeneric(this Type givenType, Type genericType)
        {
